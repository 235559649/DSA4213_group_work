...
Software engineering agents must identify relevant code before producing
a useful patch. In a large repository, an issue description may not name
the responsible files or functions, and relevant logic may be distributed
across several locations. For example, an incorrect discount calculation
may require examining input handling, pricing, and rounding functions
before identifying the cause.

This project targets bug-fixing assistance for developers and maintainers
of Python repositories. We investigate whether structured repository
information and runtime failure evidence can help mini-SWE-agent
\cite{minisweagent} locate relevant code more efficiently. An agent-based
approach is appropriate because debugging is iterative: code inspection
and execution provide evidence that informs subsequent actions.

We will retain the existing repair loop and ordinary execution feedback,
while adding ranked localization context and updates based on usable
failure evidence. The aim is to assess the additional value of this
guidance, rather than assume that more structure necessarily improves
repair.
...
\textbf{RQ2.}
Does improved localization reduce exploration costs, including file
inspections, model steps, token usage, and runtime, after accounting for
localization overhead?

\vspace{0.5em}

\textbf{RQ3.}
What additional value do repository structure and runtime failure evidence
provide beyond textual issue-to-code relevance?

\subsection*{1.3 Definition of Success}

Success would mean improved localization accompanied by better repair
performance, or comparable repair performance with lower exploration
costs. Repair success will be determined by the independent evaluation
described in Section~5; improved localization alone will not establish
successful repair.

Pilot experiments on development tasks will inform quantitative success
criteria, including the targeted improvement in localization or
exploration efficiency and the acceptable change in repair performance.
These criteria will be fixed before held-out evaluation. Comparisons and
trajectory analysis will identify benefits, trade-offs, and failure cases.
Where evidence is insufficient to establish comparable performance, we
will report that uncertainty.
...
\begin{itemize}[leftmargin=*, itemsep=0.4em]
    ...
    \item \textbf{Misleading localization.}
    Textual matches, structural relationships, and traceback locations may
    not identify the root cause. Candidate rankings may therefore
    misdirect exploration. They will serve as guidance rather than restrict
    access to other code.

    \item \textbf{Evaluation limitations.}
    Reference-patch locations are imperfect localization targets because
    alternative correct fixes may modify different code. Test-based
    evaluation also cannot establish complete correctness. Localization,
    repair outcomes, and representative trajectories will be interpreted
    together.

    \item \textbf{Benchmark contamination.}
    Public issues and solutions may have appeared in model training data.
    Separating development and evaluation tasks and withholding reference
    patches reduces experimental leakage, but cannot eliminate pretraining
    contamination.

    \item \textbf{Model variability and cost.}
    Repeated runs may produce different results, while additional
    localization can increase token usage and runtime. We will use the
    controlled settings, resource limits, and repeated-run analysis
    described in Section~5.

    \item \textbf{Environment failures.}
    Dependency or setup problems may prevent valid execution independently
    of repair quality. These failures and any resulting exclusions will be
    documented.

    \item \textbf{Limited generalizability.}
    Findings from a selected set of Python tasks and the chosen model
    configuration may not generalize to other languages, repositories, or
    models.
...
Experiments will run in task-specific containerized environments
consistent with the SWE-bench infrastructure, as described in Section~3.
Each environment will contain only the corresponding public repository,
its required dependencies, and the available testing infrastructure.
Credentials, confidential code, and unrelated host directories will be
excluded from shared materials. Repository content and tool outputs will
be treated as untrusted inputs. Generated patches will require human
review before practical use.
