...
\todo{Describe the mini-SWE-agent architecture, its minimal agent loop,
available tools, and how repository exploration is performed. Add the
appropriate citation.}

\todo{Clarify exactly which parts of mini-SWE-agent are reused without
modification and which integration points will be changed by our system.}
...
\todo{Describe the chosen SWE-bench version/subset and justify why it is
appropriate for evaluating our project. Add citation.}

\todo{Specify whether the project will use SWE-bench Verified or another
subset, and how many tasks will be used for development and evaluation.}
...
\todo{Review and cite the most relevant repository navigation /
bug localization systems. Include systems that are particularly close
to our proposed approach so that our novelty is clearly stated.}

\todo{Explain how our approach differs from prior work rather than only
listing related systems.}
...
\todo{Finalize the exact ranking algorithm before submission. For example,
decide whether C1 combines keyword similarity, semantic similarity,
repository structure, or another scoring method.}
% ====================
% 3. Agent setup
% ====================
...
\textbf{Input:}
a natural-language issue description and the corresponding repository.

\textbf{Output:}
a proposed source-code patch and an execution trajectory containing
repository exploration, edits, and test results.
...
\todo{Specify the exact environment, e.g., Docker / SWE-bench containers,
Python versions, and available system tools.}


\subsection*{3.3 Agent and Model}

The original mini-SWE-agent will be used as the software repair baseline.
Our system will augment its repository exploration process with structured
localization information.

\todo{Specify the LLM(s) used in experiments.}

\todo{Specify whether all compared systems will use the same model,
temperature, token budget, and prompts to ensure a controlled comparison.}


\subsection*{3.4 Agent Tools}

The proposed system may provide the agent with the following sources of
information:

\begin{itemize}[leftmargin=*, itemsep=0.25em]
    \item shell-based repository exploration;
    \item file, class, and function indexing;
    \item Python AST and import information;
    \item issue-to-code relevance scores;
    \item test execution results; and
    \item traceback and runtime failure evidence.
\end{itemize}

\todo{Remove any tool that is not implemented in the final system and
describe the exact interface exposed to the agent.}
...
\todo{Define the exact benchmark subset and task selection criteria.}

\todo{Specify the number of tasks used for pilot development and final
evaluation.}
...
\todo{Define maximum agent steps, token budget, runtime limit, and the
exact condition used to determine successful repair.}
...
\begin{itemize}[leftmargin=*, itemsep=0.25em]
    \item issue resolution rate;
    \item target tests passed;
    \item regression-test performance; and
    \item number of successful repairs within the resource budget.
\end{itemize}

\todo{Define the official repair-success criterion using the selected
benchmark's evaluation harness.}
...
\begin{itemize}[leftmargin=*, itemsep=0.25em]
    \item Top-1 relevant-file accuracy;
    \item Top-$k$ relevant-file accuracy;
    \item function-level localization accuracy; and
    \item rank of the ground-truth modified location.
\end{itemize}

\todo{Define ground-truth localization from benchmark patches and decide
the value(s) of $k$.}
...
\begin{itemize}[leftmargin=*, itemsep=0.25em]
    \item number of files opened;
    \item number of shell commands executed;
    \item number of agent / LLM steps;
    \item token usage;
    \item runtime; and
    \item estimated API cost.
\end{itemize}

\todo{Select the final subset of efficiency measures to avoid reporting
too many redundant metrics.}
...
\todo{Define a small failure taxonomy after observing pilot trajectories.}
...
\todo{Decide the number of repeated runs per task, subject to API and
compute cost.}

\todo{Specify model version, temperature, maximum steps, token budget,
